...
int CChatServer::SendOneSector(CPlayer* pPlayer, CPacket* pPacket, CPlayer* except)
{
	int sendCount = 0;
...
int CChatServer::SendAroundSector(CPlayer* pPlayer, CPacket* pPacket, CPlayer* except)
{
	int sendCount = 0;
...
void CChatServer::CompleteUnfinishedLogin(ULONG_PTR pStAPCData)
{
	...
	CPacket* pSendPacket = chatServer.AllocPacket();
	do  // do..while(0)
	{
		if (isNull == true)
		{
