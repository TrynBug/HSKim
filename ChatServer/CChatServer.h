...
private:
	...
	CLockfreeQueueTLS<MsgChatServer*> _msgQ;
...
private:
	...
	/* 네트워크 전송 */
	int SendUnicast(__int64 sessionId, CPacket* pPacket);
	int SendUnicast(CPlayer* pPlayer, CPacket* pPacket);
	int SendBroadcast(CPacket* pPacket);
	int SendOneSector(CPlayer* pPlayer, CPacket* pPacket, CPlayer* except);
	int SendAroundSector(CPlayer* pPlayer, CPacket* pPacket, CPlayer* except);
